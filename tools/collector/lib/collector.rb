  ...
  class Collector
    ...
    def initialize
      ...
      @nats = NATS.connect(:uri => Config.nats_uri) do
        ...
        @nats.subscribe(@inbox) do |message|
          component_latency = ((Time.now.to_f - @component_discover_sent) * 1000).to_i
          process_component_discovery(message, component_latency)
        end

        @component_discover_sent = Time.now.to_f
        @nats.publish(DISCOVER_SUBJECT, "", @inbox)

        @colector_instance = UUID.generate.to_s
        @nats.subscribe("collector.nats.ping.#{@colector_instance}") do |message|
    ...
    def setup_timers
      ...
      EM.add_periodic_timer(Config.nats_ping_interval) do
        @nats.publish("collector.nats.ping.#{@colector_instance}", Time.now.to_f.to_s)
      end
    end
    ...
    def process_component_discovery(message, latency = nil)
      ...
      if message["uuid"]
        ...
        instances[message["uuid"]] = {
          #:host => message["host"],
          #:credentials => message["credentials"],
          :timestamp => Time.now.to_i,
          :announce_latency => latency,
          :vcap_component => message
        }
      end
    rescue Exception => e
    ...
    def fetch_varz
      @components.each do |job, instances|
        instances.each do |uuid, instance|
          next unless credentials_ok?(job, instance)
          varz_uri = "http://#{instance[:vcap_component]["host"]}/varz"

          request_start = Time.now.to_f
          http = EventMachine::HttpRequest.new(varz_uri).get(
                  :head => authorization_headers(instance))
          http.errback do
            @logger.warn("Failed fetching varz from: #{instance[:vcap_component]["host"]}")
          end
          http.callback do
            begin
              json_varz = http.response.sub(/^\xEF\xBB\xBF/, '')
              varz = Yajl::Parser.parse(json_varz)

              instance[:varz] ||= {}

              # this is a fix for dea
              if job == "DEA"
                if instance[:varz][:data] && instance[:varz][:data]['running_apps']
                  if varz['num_apps'] && varz['num_apps'] != 0 && varz['running_apps'] && varz['running_apps'].count == 0
                    varz['running_apps'] = instance[:varz][:data]['running_apps']
                  end
                end
              end

              instance[:varz][:request_latency] = ((Time.now.to_f - request_start) * 1000).to_i
              instance[:varz][:data] = varz



              now = Time.now.to_i

              handler = Handler.handler(@tsdb_connection, job, instance[:vcap_component]["index"], now)
              if varz["mem"]
    ...
    def fetch_healthz
      @components.each do |job, instances|
        instances.each do |uuid, instance|
          next unless credentials_ok?(job, instance)
          healthz_uri = "http://#{instance[:vcap_component]["host"]}/healthz"

          request_start = Time.now.to_f
          http = EventMachine::HttpRequest.new(healthz_uri).get(
                  :head => authorization_headers(instance))
          http.errback do
            @logger.warn("Failed fetching healthz from: #{instance[:vcap_component]["host"]}")
          end
          http.callback do
            begin
              now = Time.now.to_i
              ...
              instance[:healthz] ||= {}
              instance[:healthz][:request_latency] = ((Time.now.to_f - request_start) * 1000).to_i
              instance[:healthz][:data] = http.response
              instance[:healthz][:is_healthy] = instance[:healthz][:data].strip.downcase == "ok" ? true : false
    ...
    def credentials_ok?(job, instance)
      unless instance[:vcap_component]["credentials"].kind_of?(Array)
    ...
    def authorization_headers(instance)
      credentials = Base64.strict_encode64(instance[:vcap_component]["credentials"].join(":"))
